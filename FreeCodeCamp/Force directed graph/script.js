...
const infoUrl = 'https://restcountries.eu/rest/'

const CHART_HEIGHT = 700,
      CHART_WIDTH = 1200,
      NODE_HEIGHT = 25,
      NODE_WIDTH = 30,
      LINK_DIST = 100, // target distance between nodes
      CHARGE = -70;    // repulsion or attration str
...
d3.json(dataUrl, (err, data) => {
  ...
  const force = d3.layout.force()
                  .nodes(data.nodes) // set nodes
                  .links(data.links) //set links
                  .size([CHART_WIDTH, CHART_HEIGHT]) // available size
                  .linkDistance([LINK_DIST]) // target distance between nodes
                  .charge([CHARGE]) // repulsion or attration
                  .on('tick', tickHandler)
                  .start();
  
  const nodes = d3.select('.chart-wrap')
                  .selectAll('element')
                  .data(data.nodes)
                  .enter()
                  .append('element')
                  ...
                  .style({
                     'position': 'absolute',
                     'height': d => NODE_HEIGHT + 'px',
                     'width': d => NODE_WIDTH + 'px',
                  })                  
                  .call(force.drag);
  /*
  nodes.append('text')
  ...
  */
  ...
  function tickHandler() {    
    ...
    links.attr({
            'x1': d => d.source.x + NODE_WIDTH / 2,
            'y1': d => d.source.y + NODE_HEIGHT / 2,
            'x2': d => d.target.x + NODE_WIDTH / 2,
            'y2': d => d.target.y + NODE_HEIGHT / 2   
          })
  }
  
  
  d3.json(infoUrl, (err, rawInfo) => {
     if(err) throw err;
    // parse
    const info = {};
    rawInfo.forEach((country) => {
      info[country['alpha2Code']] = country;
    });    
    ...
    nodes.on('mouseover', (d) => {
      ...
      // update toolTip
      const countryCode = d.code.toUpperCase();
      const { nationality, capital, population, region, currency } = info[countryCode];
      
      d3.select('.flag-box').attr( 'class', d => ' flag-box flag flag-' + countryCode.toLowerCase() );
      d3.select('.country .name').text( d.country + ' - ' + countryCode );
      d3.select('.nationality').text( 'Nationality: ' + nationality );
      d3.select('.capital').text( 'Capital City: ' + capital );
      d3.select('.population').text( 'Population: ' + population.toLocaleString() );
      d3.select('.region').text( 'Region: ' + region );
      d3.select('.currency').text( 'Currency: ' + currency );
...
// DATA BACKUP because who knows if it will still there : /
/*
